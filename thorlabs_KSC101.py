...
if __name__ == '__main__':
    shutter = Controller(
        'COM18', mode='manual', verbose=True, very_verbose=False)

##    shutter.identify()

    print('\n# Manual mode open and close shutter: (blocking calls)')
    shutter.set_state('open')
    shutter.set_state('closed')

    print('\n# Manual open and close shutter: (non-blocking calls)')
    shutter.set_state('open', block=False)
    shutter.set_state('closed', block=False)
    print('(immediate follow up call forces finish on pending move)')
    print('doing something else')
    shutter._finish_set_state()

    print('\n# Single mode: (minimally supported)')
    shutter.set_mode('single')
    shutter.set_state('open')
    print(' -> add the appropriate delay here...')
    shutter.set_state('closed')

    print('\n# Auto mode: (minimally supported)')
